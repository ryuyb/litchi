...
function StatsCard({
	...
	return (
		<div className="glass-card rounded-2xl p-6 relative overflow-hidden group">
			<div
				className={`absolute top-0 right-0 p-4 opacity-10 transition-transform duration-500 group-hover:scale-125 ${colorClass}`}
			>
				<Icon size={80} />
			</div>
	...
	return (
		...
		>
			...
			<div className="flex items-center gap-4">
				<span
					className={`px-2.5 py-1 rounded-full text-xs font-semibold tracking-wide ${stageColor} shadow-sm`}
				>
					{stageInfo?.label ?? "Unknown"}
				</span>
				<ChevronRightIcon
					size={16}
					className="text-muted-foreground opacity-0 -ml-2 group-hover:opacity-100 group-hover:ml-0 transition-all"
				/>
			</div>
		</div>
	)
			...
			<section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary/90 via-primary to-primary/80 px-8 py-12 shadow-xl shadow-primary/20 text-primary-foreground border border-primary-foreground/10">
				...
				<div className="relative z-10 max-w-2xl">
					...
					<p className="text-lg text-primary-foreground/90 font-medium leading-relaxed max-w-xl">
						Your automated AI development agent. Seamlessly connecting GitHub
						Issues to brilliant Pull Requests with intelligent code generation.
					</p>

					<div className="mt-8 flex gap-4">
						<button
							type="button"
							className="bg-background text-foreground px-6 py-2.5 rounded-full font-semibold text-sm shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all"
						>
							New Session
						</button>
						<button
							type="button"
							className="bg-primary-foreground/20 backdrop-blur-md text-primary-foreground border border-primary-foreground/30 px-6 py-2.5 rounded-full font-semibold text-sm hover:bg-primary-foreground/30 transition-all"
						>
							View Documentation
						</button>
					</div>
							...
							{isLoading ? (
								...
								<div className="glass-card rounded-2xl p-10 flex flex-col items-center justify-center text-center border-dashed">
									...
									<p className="text-muted-foreground max-w-sm">
										There are currently no active development sessions. Start a
										new workflow to see activity here.
									</p>
								</div>
							)}
						</div>
				...
				<div className="space-y-8">
					<section className="glass-card rounded-3xl p-6 shadow-sm border-t-4 border-t-primary">
						...
						<p className="text-sm text-muted-foreground mb-6">
							Each AI development session progresses systematically through the
							following stages:
						</p>
						<div className="space-y-4">
							{workflowStages.map((stage, i) => (
								<div
									key={stage.key}
									className="flex gap-4 relative animate-slide-up-fade"
									style={{ animationDelay: `${i * 100}ms` }}
								>
									<div className="flex flex-col items-center">
										<div
											className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-md z-10 ${i === workflowStages.length - 1 ? "bg-emerald-500" : "bg-primary"}`}
										>
											{stage.icon}
										</div>
					...
					<section className="relative rounded-3xl bg-card overflow-hidden border border-border p-6 shadow-sm group">
						...
						<p className="text-xs text-muted-foreground mt-4">
							Agent workers are connected and ready to accept new issues.
						</p>
					</section>
				</div>
			</div>
